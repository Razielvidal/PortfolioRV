export default function SkillsCard() {
  ...
  // New tools array with your logos and names
  const tools = [
    { name: "Excel", icon: "/excel%20logo.png" },
    { name: "Jira", icon: "/jira%20logo.png" },
  ...
  return (
    ...
    >
      ...
      {/* Divider between Skills & Tools */}
      <div className="border-t border-gray-300 dark:border-gray-600 mt-4 pt-4">
        <h2 className="text-xl font-bold text-gray-800 dark:text-white">Tools &amp; Platforms</h2>
        <div className="flex flex-wrap gap-4 mt-2">
          {tools.map((tool) => (
            <div key={tool.name} className="flex items-center gap-2">
              <img src={tool.icon} alt={`${tool.name} Logo`} className="w-6 h-6" />
